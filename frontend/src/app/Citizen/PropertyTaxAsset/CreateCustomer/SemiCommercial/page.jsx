'use client'
import React, { useState, useRef, useContext, Suspense, useEffect } from 'react'
import { Admin } from "@/app/AdminContext/AdminManageMent";
import { useRouter } from "next/navigation";
const page = () => {
  const [activeTab, setActiveTab] = useState("semi-commercial"); // State for switching tabs
  ...
  const handleTabChange = (tab) => {
    setActiveTab(tab);
    if (tab === "residential") {
      router.push("../CreateCustomer"); // Navigate to commercial page
    }
  };
  ...
  async function getlocation() {
    if (typeof navigator != undefined) {
      navigator.geolocation.getCurrentPosition((x) => {
        ...
        let val = lat + '/' + long
        console.log("Location vlaue :: ", val);
        setFormData({
          ...formData,
          ['location']: val,
        });
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    if (Number(formData.TotalArea) > Number(formData.Area_Shop)) {
      try {
        console.log("Locatiyt :: ", formData)
        ...
        if (!Verification) {
          ...
          let res = await CreateCustomer(uploadform);
          alert(res.message)
      ...
      alert("Area of House Should greater than Area of Shop !! Edit it !!");
    }
  ...
  useEffect(() => {
    ...
    if (localityrate && formData.TotalArea) {
      totalARV = Number(localityrate) * Number(formData.Area_Use) * 12 * 3;
    }
        ...
        <div >
          ...
          <form ref={formref} onSubmit={handleSubmit} style={{ display: 'flex', width: '100%', flexWrap: "wrap", padding: '3%', borderRadius: '20px', backgroundColor: 'white', marginTop: '10px' }} autoComplete="off">
            ...
            <div className="mx-2" style={{ width: '45%', padding: '8px' }} >
              ...
              <input autoComplete="off" type="text" name="PanNumber" value={formData.PanNumber} placeholder="Pan Number" onChange={handleChange} onBlur={handleBlur} className='form-control mt-2' required />
            </div>
            ...
            <div className="mx-2" style={{ width: '45%', padding: '8px' }} >
              <label style={{ fontWeight: 'bold' }}>Area Of house</label><br />
            ...
            <div className="mx-2" style={{ width: '45%', padding: '8px' }} >
              <label style={{ fontWeight: 'bold' }}>Area Of Use of house</label><br />
              <input autoComplete="off" type="text" name="Area_Use" value={formData.Area_Use} placeholder="Area use" onChange={handleChange} className='form-control mt-2' required />
            </div>
            <div className="mx-2" style={{ width: '45%', padding: '8px' }} >
              <label style={{ fontWeight: 'bold' }}>Area Of Shop</label><br />
              <input autoComplete="off" type="text" name="Area_Shop" value={formData.Area_Shop} placeholder="Area Shop" onChange={handleChange} className='form-control mt-2' required />
            </div>
            {/* <div className="mx-2" style={{ width: '45%', padding: '8px' }} >
            ...
            <div className="mx-2" style={{ width: '45%', padding: '8px' }} >
              ...
              {/* <select className="form-control mt-2" name="PropertyType" onChange={(e) => { handleChange(e) }} required>
                ...
                <option disabled selected className="form-control mt-2" >Commercial</option>
              
              </select> */}

              <input type="text" value={"Commercial"} className="form-control py-2" disabled />
            </div>
            <div className="mx-2" style={{ width: '45%', padding: '8px' }} >
            ...
            <div className="mx-2" style={{ width: '45%', padding: '8px' }} >
              ...
              <input type="text" name="TotalARV" value={formData.TotalARV} placeholder="Total ARV" onChange={handleChange} className='form-control mt-2' required />
            </div>
            <div className="mx-2" style={{ width: '45%', padding: '8px' }} >
              <label style={{ fontWeight: 'bold' }}>Floor Number</label><br />
              <input autoComplete="off" type="text" name="Talo_ki_Sankhya" value={formData.Talo_ki_Sankhya} placeholder="Floor Number" onChange={handleChange} className='form-control mt-2' required />
            </div>
            <div className="mx-2" style={{ width: '45%', padding: '8px' }} >
