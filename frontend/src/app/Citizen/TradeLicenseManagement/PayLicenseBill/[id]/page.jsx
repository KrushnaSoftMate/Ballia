...
const page = ({ params }) => {
  ...
  const { GetLicenseBills, GetLicenseHashKey, CustomerLicensePayment } = statecalls;
  ...
  const getBill = async () => {
    const data = await GetLicenseBills(params.id);
  ...
  async function GetHashKeyData(x) {
    ...
    let haskey = await GetLicenseHashKey(obj);
      ...
      console.log(renderdata);
        ...
        var options = {
          ...
          onResponse: async (response1) => {
            ...
            await CustomerLicensePayment(obj1);
            if (response1.status === "success") {
              alert('Payment Success full')
              window.location.href = `/Citizen/LicenceReciept/${params.id}/`
            }
          },
          theme: "#123456",
        };
