...
const CustomerBill = (props) => {
    ...
    const { GetLicenseBills } = statecalls;
    ...
    const getBill = async (e) => {
        e.preventDefault();
        if (billdata.Gala) {
            const data = await GetLicenseBills(billdata.Gala);
            console.log(data);
            if (data.length > 0) {
                SetLicenseBill(data)
            } else {
            ...
            >
                <form onSubmit={getBill} style={{ height: '13vh', display: "grid" }}>
                    ...
                    <input
                        ...
                        type="text"
                        id="Gala"
                        placeholder="Enter Shop Number Here"
                        onChange={(e) =>
                            setBillData({ ...billdata, [e.currentTarget.id]: e.target.value })
                        }
                    />
                    <div style={{ display: "flex", justifyContent: "center", marginTop: '15px' }}>
                        <button style={{ backgroundColor: '#ec931f', color: "white" }} className="my-2 mx-2 btn">Get License Bill</button>
                    </div>
                </form>
            </div>

            <div hidden={licensebill[0]?.BillNumber === undefined ? true : false}
                style={{
                    display: 'grid',
                    border: '2px solid',
                    borderRadius: '20px',
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <button className="btn" style={{ backgroundColor: '#f1772e' }}>
                                            ...
                                            <Link style={{ color: 'white', textDecoration: 'none', margin: '8px' }} href={"/Customer/LicenceBill/" + e.Gala}>
                                               View Bill
                                            </Link>
                                        </button>
                                    </td>
